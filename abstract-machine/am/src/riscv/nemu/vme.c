...
void map(AddrSpace *as, void *va, void *pa, int prot) {
  ...
  if (!(*page_table_entry & PTE_V)){ // 说明二级表未分配
    ...
    *page_table_entry = (*page_table_entry | PTE_V);
    ...
    if ((PTE_PPN(*page_table_entry) * 4096 + VA_VPN_1(va) * 4) != (uintptr_t)alloced_page){
      printf("%p\t%p\n", (PTE_PPN(*page_table_entry) * 4096 + VA_VPN_1(va) * 4), alloced_page);
    }
    assert((PTE_PPN(*page_table_entry) * 4096 + VA_VPN_1(va) * 4) == (uintptr_t)alloced_page);
  }
